...
class DocxExporter:
    ...
    def _get_content_width_in(self) -> float:
        """Returns printable width in inches."""
        p = self.template.page
        inside = getattr(p, "margin_inside_in", 0.85) or 0.85
        outside = getattr(p, "margin_outside_in", 0.7) or 0.7
        width = getattr(p, "width", 5.83) or 5.83
        return max(3.0, width - inside - outside)
    ...
    def _write_toc_entry_line(self, doc: docx.Document, block: Block, content_width_in: float) -> None:
        """Formats a compact, elegant Table of Contents entry with dot leaders."""
        ...
        # Add dot leader tab stop at right margin
        try:
            pf.tab_stops.add_tab_stop(Inches(content_width_in), WD_TAB_ALIGNMENT.RIGHT, WD_TAB_LEADER.DOTS)
        except Exception:
            pass

        # Determine structural level
        is_part = text.startswith("PART ")
        is_chapter = re.match(r"^(?:Chapter\s+\d+|CHAPTER\s+\d+)", text, re.I) is not None
        is_section = re.match(r"^(?:Section\s+\d+\.\d+|\d+\.\d+\s+)", text, re.I) is not None
        is_subsection = re.match(r"^(?:Subsection\s+\d+\.\d+\.\d+|\d+\.\d+\.\d+)", text, re.I) is not None
    ...
    def _write_block(
        ...
        # 1. Handle Table
        if block.block_type == BlockType.TABLE and "table_data" in block.metadata:
            self._write_table(doc, block.metadata["table_data"])
            return

        # 2. Check for explicit page breaks (ONLY on PART_TITLE and CHAPTER_TITLE)
        if is_main_matter and not is_first_in_section:
            if block.block_type == BlockType.PART_TITLE:
                doc.add_page_break()
            elif block.block_type == BlockType.CHAPTER_TITLE:
                # If Part immediately preceded this Chapter, share the opener page
        ...
        if block.block_type == BlockType.PART_TITLE:
        ...
        elif block.block_type in (BlockType.HEADING_2, BlockType.HEADING_3):
            p.style = "Heading 3"
            pf.alignment = WD_ALIGN_PARAGRAPH.LEFT
        ...
        elif block.block_type in (BlockType.CAPTION, BlockType.TABLE_CAPTION):
            pf.alignment = WD_ALIGN_PARAGRAPH.CENTER
            pf.space_before = Pt(5)
            pf.space_after = Pt(6)
            pf.keep_with_next = True
            font_size = 9.0
            is_bold = (block.block_type == BlockType.TABLE_CAPTION)
            ...
            pf.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
            pf.space_before = Pt(0)
            pf.space_after = Pt(4.5)
            pf.line_spacing = 1.15
            font_size = 11.0

            # First-line indent (0 for first paragraph after a heading, 0.2" otherwise)
    ...
    def _write_table(self, doc: docx.Document, table_data: list) -> None:
        ...
        content_w = self._get_content_width_in()
        col_max_lens = [0] * cols
        ...
        shading_xml = r'<w:shd %s w:fill="F4F5F7"/>' % nsdecls("w")

        # Table Borders XML (Booktabs publication style)
        borders_xml = (
            r'<w:tblBorders %s>'
            r'<w:top w:val="single" w:sz="8" w:space="0" w:color="333333"/>'
            r'<w:bottom w:val="single" w:sz="8" w:space="0" w:color="333333"/>'
